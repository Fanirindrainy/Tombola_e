...
class Alea3Page extends StatefulWidget {
  @override
  State<Alea3Page> createState() => _Alea3PageState();
}

List<int> selectedChiffresBillet3 = [];

class _Alea3PageState extends State<Alea3Page> {
  ...
  Future<void> fetchData() async {
    ...
    if (responseBillet3.statusCode == 200) {
      selectedChiffresBillet3 =
          List<int>.from(jsonDecode(responseBillet3.body));
    }
  }
  ...
  void _startButtonPressed() {
    ...
    randomChiffres =
        generateRandomNumbers(selectedChiffresBillet3, numberOfBilletsInt);
    setState(() {
  ...
  void _showConfirmationAlert() {
    ...
    try {
    final response =
        await TroisVenduService.vendreBillets(
            randomChiffres);
