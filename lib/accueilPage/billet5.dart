...
import 'package:tombola/service/selection.dart';
import 'dart:convert';
...
class Billet5Page extends StatefulWidget {
  @override
  State<Billet5Page> createState() => _Billet5PageState();
}
List<int> selectedChiffresBillet5
 = [];

class _Billet5PageState extends State<Billet5Page> {
  ...
  Future<void> fetchData() async {
    final responseBillet3 = await CinqService.getCinqFromServer();
    if (responseBillet3.statusCode == 200) {
      setState(() {
        selectedChiffresBillet5 = List<int>.from(jsonDecode(responseBillet3.body));
       
      });
     while (selectedIndexesByPage.length < (selectedChiffresBillet5.length / itemsPerPage).ceil()) {
  ...
  List<int> getBillet3List(int currentPage, int itemsPerPage) {
    ...
    if (endIndex > selectedChiffresBillet5.length) {
      endIndex = selectedChiffresBillet5.length;
    }

    return selectedChiffresBillet5.sublist(startIndex, endIndex);
  }
                ...
                children: [
                  ...
                  IconButton(
                    onPressed: currentPage <
                            (selectedChiffresBillet5.length - 1) ~/ itemsPerPage
                        ? () => goToPage(currentPage + 1)
                        : null,
